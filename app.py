...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS SCHEDULE (Id serial,Tanggal_Periksa date, Nama varchar, Jenis_Kelamin char(25), Umur text, Pekerjaan varchar,\
...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO schedule (Id,Tanggal_Periksa, Nama, Jenis_Kelamin, Umur, Pekerjaan, No_Telp, Alamat, Dokter, Hasil_Diagnosa) \
                    ...
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            ...
                            session.execute(query, {'1':Tanggal_Periksa_baru, '2':Nama_baru, '3':str(Jenis_Kelamin_baru), 
                                                    '4':Umur_baru, '5':Pekerjaan_baru, '6':No_Telp_baru, '7':Alamat_baru, '9':str(Dokter_baru), '10':Id})
                            session.commit()
